...
class FixedTrimFileManager(object):
    """FixedTrimFileクラスを管理する"""
    ...
    def setCurrent(self, index):
        """ FixedTrimFileインスタンスを選択
        @param index 選択するFixedTrimFileインスタンスの番号 """
        self.current = self.files_data[index]

    def getCurrent(self):
        """ 選択中のFixedTrimFileインスタンスを取得
        @return 選択中のFixedTrimFileインスタンス """
    ...
    def getImagesUsingIndex(self, index):
        """ インデックスを指定してその番号の画像を全てのFixedTrimFileインスタンスから取得するジェネレーターを返す
    ...
    def saveFile(self, isMakeNewFile = False, FTparent = None):
        if self.fname is None or isMakeNewFile:
            self.fname, _ = QFileDialog.getSaveFileName(parent = FTparent, caption = "Save File" , filter ='Fixed Trim Data (*.ftd)',
                directory = './FixedTrimData({}).ftd'.format(
    ...
    def importDataList(self, dlist):
        new_data = FixedTrimFile.fromDataList(*dlist)
        if new_data == -1:
            return -1
    ...
    def getOpenDataFileDilalog(ft_widget = None):
        return QFileDialog.getOpenFileName(parent = ft_widget, caption = "Open File" , filter ='Fixed Trim Data (*.ftd)' )[0]
    @staticmethod
    def fromFile(fname = None, ft_widget = None):
        """ファイルからインスタンスを設定"""
        if fname is None:
            fname = FixedTrimFileManager.getOpenDataFileDilalog(ft_widget)
        if fname == '':
            return -1
        path = Path(fname)
        ret = FixedTrimFileManager()
