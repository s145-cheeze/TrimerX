...
class FTILS_Result(Enum):
    """ FixedTrimMainクラスにOKかキャンセルかを伝える列挙型
...
class FixedTrimImportListSetting(QWidget):
    """トリミングしたい画像を指定する"""
    def __init__(self,fmanager ,parent = None):
        super(FixedTrimImportListSetting, self).__init__(parent)
        self.ft_main = None
        self.index = -1
        self.fmanager = fmanager
        self.initUI()
    def connectFTMain(self, arg):
        """ FixedTrimMainインスタンスを設定します
        設定する理由：FixedTrimMainに決定かキャンセルか伝えるため """
        self.ft_main = arg
...
def main():
    ...
    ex1 = FixedTrimImportListSetting()
    ex1.show()
    sys.exit(app.exec_())
